...
a window that never paints. Tauri hands linuxdeploy a fixed argument list and
will not spawn a wrapper script in its place, so the job unpacks the finished
image, deletes the four files and packs it again with the appimage plugin
Tauri caches (appimagetool over the same AppDir). The job checks the extracted image for a
