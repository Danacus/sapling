 ...
 * (`./escalation`). `match-pairs` never costs a token at all, but it is not
 * generation either: it is built locally in
 * `$lib/challenges/local/match-pairs` and only spliced in by the session.
 *
...
import { undefinedIfBlank } from './resolve-helpers';
import {
	batchJsonSchemaFor,
	batchSchemaNameFor,
