...
**The Worker is a sequencer, nothing else.** `worker/` (`wrangler.jsonc`) assigns every event a `seq` and hands events back in that order; it never merges and never reads a payload — every merge rule lives in `src/lib/db/materialize.ts`. The pairing phrase travels as an `Authorization: Bearer` header, normalised and hashed to name the Durable Object room. `worker/` imports `src/lib/sync/phrase.ts` from the client on purpose, so the two normalisations can never drift apart — which means **Workers Builds *build watch paths* must include `src/lib/sync/*`, not just `worker/*`**, or a new client can ship against a stale Worker; the symptom is a second device opening an empty library, which reads as lost data rather than a version skew. Leaving watch paths empty (rebuild on every push) is the safe default. The app finds the Worker through `VITE_SYNC_URL`, a build-time variable set in the Pages project's environment; unset, the build simply has no sync. `pnpm check` typechecks the Worker too (`tsc -p worker`), since svelte-check's tsconfig only covers `src/**`. See `docs/sync.md`.
