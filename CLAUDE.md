...
| `crates/sapling-desktop/` | A **spike**, and a host only: the same core over a SQLite file, the same `Backend` protocol, no merge rule and no SQL of its own. It also lends **speech both ways** (one feature, `speech`) — Kokoro out, SenseVoice in — over one shared `models.rs`; the microphone stays in the webview. And it lends **a video's captions**, `#[cfg(desktop)]`: it runs `yt-dlp` off PATH (never pinned — it ages against YouTube in weeks) and hands back raw `json3` it never parses, which closes the one thing reading mode could not do from a page. Not a workspace default member; needs `nix develop .#desktop`. Also builds in CI as an Android **release** APK signed from repository secrets, where it is the same host **minus the player and the captions**: synthesis and recognition are native there too, only `tts::play`/`tts_play`/`tts_stop` read `all(feature = "speech", desktop)`, and `tts_status`'s `playback` is how the window learns which it got. | `desktop.md` |
