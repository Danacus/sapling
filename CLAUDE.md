...
| `crates/sapling-desktop/` | A **spike**, and a host only: the same core over a SQLite file, the same `Backend` protocol, no merge rule and no SQL of its own. Not a workspace default member; needs `nix develop .#desktop`. Also builds in CI as an Android **release** APK signed from repository secrets, where it is the same host **minus the voice** — gates read `all(feature = "tts", desktop)`, never the feature alone. | `desktop.md` |
